...
Based on the codebase, here's how the neon_auth.users_sync table is used and referenced:

Schema Definition
The table is defined in shared/schema.ts as a reference to Neon Auth's managed schema:
...
1. Authentication Flow (server/neonAuth.ts)
The middleware extracts the user ID from Stack Auth's headers and attaches it to requests:

const userId = req.headers['x-stack-user-id'] as string;
(req as any).neonUser = { id: userId };
2. Lazy User Creation (server/routes/auth.ts & server/routes/user.ts)
When a user first accesses the app, the application:

Checks if they exist in the local users table
If not, queries neon_auth.users_sync to get their profile data
Creates a record in the local users table with data from Neon Auth
Example from server/routes/auth.ts:
...
3. Data Flow
Neon Auth manages: neon_auth.users_sync table (automatically synced with Stack Auth)
Your app manages: users table for app-specific data (subscriptions, credits, etc.)
The link: User ID is shared between both tables, but no foreign key constraint exists
Key Points:
The neon_auth.users_sync table is read-only for your application
It's automatically maintained by Neon Auth
Your app queries it only to sync user profile data (email, name) when creating local user records
The local users table stores app-specific data and uses the same user ID as the primary key
This separation allows Neon Auth to handle authentication while your app maintains its own user-related business logic and data.