  ...
  const ChatWidget = {
    ...
    init: function (options = {}) {
      ...
      if (!this.config.apiUrl.includes("/widget/")) {
        console.error(
          "Chat widget: API URL must include /widget/ path for proper functionality",
        );
        return;
      }
    ...
    loadStylesAndCreateWidget: function () {
      ...
      link.onerror = () => {
        console.warn(
          "Chat widget CSS failed to load, creating widget without styles",
        );
        this.createWidget();
        this.setupEventListeners();
      };
    ...
    safeSessionStorage: {
      getItem: function (key) {
        try {
          ...
          console.warn(
            "sessionStorage not accessible in embed, using session-based fallback",
          );
          return null;
        }
      },
      setItem: function (key, value) {
        try {
          ...
          console.warn(
            "sessionStorage not accessible in embed, skipping storage",
          );
        }
      },
      removeItem: function (key) {
        try {
          ...
          console.warn(
            "sessionStorage not accessible in embed, skipping removal",
          );
        }
      },
    },
    ...
    setupEventListeners: function () {
      ...
      const messageBubble = document.getElementById(
        "chatwidget-message-bubble",
      );
        ...
        if (isMobile()) {
          ...
          if (!mobileIframe.src) {
            try {
              ...
              mobileIframe.onload = () => {
                ChatWidget.logDebug("mobile iframe load", {
                  src: mobileIframe.src,
                });
              ...
              mobileIframe.src = this.forceHttps(
                `${this.config.apiUrl}?embedded=true&mobile=true`,
              );
            }
          }
          ...
          if (!iframe.src) {
            try {
              ...
              // Fallback URL construction
              iframe.src = this.forceHttps(
                `${this.config.apiUrl}?embedded=true`,
              );
            }
          }
      ...
      window.addEventListener("message", (event) => {
        ...
        } else if (event.data.type === "RESET_SESSION") {
          try {
            const newId = event.data.sessionId;
            // Update parent-side embed session storage
            ChatWidget.safeSessionStorage.setItem("embed-session-id", newId);

            // Update iframe URLs to use the new sessionId if currently loaded
            const updateUrlWithSession = (src) => {
              if (!src) return src;
              const hasParam = src.includes("sessionId=");
              const separator = src.includes("?") ? "&" : "?";
              if (hasParam) {
                return src.replace(
                  /sessionId=[^&]+/,
                  `sessionId=${encodeURIComponent(newId)}`,
                );
              }
              return `${src}${separator}sessionId=${encodeURIComponent(newId)}`;
            };

            if (iframe && iframe.src) {
              iframe.src = ChatWidget.forceHttps(
                updateUrlWithSession(iframe.src),
              );
            }
            if (mobileIframe && mobileIframe.src) {
              mobileIframe.src = ChatWidget.forceHttps(
                updateUrlWithSession(mobileIframe.src),
              );
            }
          } catch (e) {
            // Swallow errors to avoid disrupting the page
          }
        }
      });
      ...
      window.addEventListener("resize", () => {
        ...
        resizeTimeout = setTimeout(() => {
          ...
          ChatWidget.logDebug("resize handler (layout change)", {
            nowMobile,
            isOpen,
            overlayDisplay: overlay.style.display,
            iframeVisible: iframe.style.visibility,
            mobileVisible: mobileIframe.style.visibility,
          });
          ...
          if (isOpen) {
            if (nowMobile && overlay.style.display === "none") {
              ...
              if (!mobileIframe.src && iframe.src) {
                ...
                let mobileUrl = currentSrc.replace(
                  "embedded=true",
                  "embedded=true&mobile=true",
                );
              ...
              if (!iframe.src && mobileIframe.src) {
                ...
                let desktopUrl = currentSrc.replace(
                  /(&|\?)mobile=true/,
                  "$1embedded=true",
                );
                // Ensure sessionId is preserved if not already in URL
    ...
    loadInitialMessages: function (messageBubble) {
      ...
      const urlMatch = this.config.apiUrl.match(
        /\/widget\/([^\/]+)\/([^\/\?]+)/,
      );
      ...
      if (urlMatch && urlMatch[1] && urlMatch[2]) {
        ...
        this.fetchWithTimeout(
          `${baseUrl}/api/public/chatbot/${userId}/${chatbotGuid}`,
          5000,
        )
          .then((response) => {
            if (!response.ok) {
          ...
          .catch((error) => {
            ...
            console.log(
              "Chat widget: Error fetching chatbot config, keeping widget hidden",
            );
          });
      } else {
    ...
    displayInitialMessages: function (messageBubble, messages) {
      ...
      try {
        const areDismissed =
          this.safeSessionStorage.getItem(dismissedKey) === "true";
        if (areDismissed) {
      ...
      const existingStack = document.getElementById(
        "chatwidget-initial-message-stack",
      );
      if (existingStack) {
      ...
      closeWrapper.style.justifyContent =
        this.config.position === "bottom-right" ? "flex-end" : "flex-start";
      ...
      messages.forEach((message, index) => {
        ...
        setTimeout(() => {
          ...
          individualBubble.style.boxShadow =
          ...
          const measuredWidth = Math.max(
            individualBubble.getBoundingClientRect().width,
            280,
          );
    ...
    hideInitialMessage: function (index) {
      if (index !== undefined) {
        ...
        const bubbleWrapper = document.querySelector(
          `.chatwidget-initial-message-item[data-index="${index}"]`,
        );
        if (bubbleWrapper) {
          ...
          setTimeout(() => {
            ...
            if (stack) {
              const messageItems = stack.querySelectorAll(
                ".chatwidget-initial-message-item",
              );
    ...
    hideAllInitialMessages: function () {
      ...
      const allBubbles = stack.querySelectorAll(
        ".chatwidget-initial-message-item",
      );
