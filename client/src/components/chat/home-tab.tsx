  ...
  if (hasCustomHomeScreen) {
    const handleTopicClick = (topic: any) => {
      ...
      if (topic.actionType === "survey") {
        ...
        const surveyPayload = {
          ...
          internalMessage: `Let's begin a New survey, ignoring previous surveys. User message:${topic.title} (surveyId: ${topic.surveyId})`
        };
    ...
    const handleActionClick = (action: any) => {
      ...
      if (action.action === "take_assessment" || action.actionType === "survey") {
        ...
        onStartChat(action.title, action.description || `I'm interested in ${action.title}`);
      }
    };
          ...
          <div className="grid grid-cols-1 gap-3">
            ...
            <Button
              ...
              onClick={() =>
                onStartChat(
                  "Live Agent",
                  "I would like to speak with a human agent please.",
                )
              }
              variant="outline"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span
                        className={`text-xs px-2 py-1 rounded-full border ${categoryColors[topic.category]}`}
                      >
                        {topic.category}
                      </span>
                    </div>
